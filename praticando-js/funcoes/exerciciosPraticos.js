...
function avaliarDesempenho(pontuacao, callbackmsg) {
  let status = "";
  ...
  callbackmsg(pontuacao, status);
}

function mensagemResultado(pontuacao, status) {
...
avaliarDesempenho(82, mensagemResultado);
...
function sorteioNome(nome) {
  const sorteio = Math.floor(Math.random() * nome.length);
  return nome[sorteio];
}
const mensagemAtraso = (sorteado) => {
  console.log("Sorteando...");
  setTimeout(() => {
    console.log("E o sorteado foi: " + sorteado.nome);
    console.log("Pontuação: " + sorteado.pontuacao);
    console.log(avaliacao(sorteado.pontuacao));
  }, 2000);
};
...
function realizarSorteio(dados) {
  const participante = sorteioNome(dados);
  mensagemAtraso(participante);
}
...
function contagemRegressiva(numInt) {
  if (numInt > 0) {
    console.log(numInt);
    contagemRegressiva(numInt - 1);
  } else {
